...
export default function BrowsePageLayout({
  ...
  return (
    <>
      <FilterDrawer />
      <main className="container mx-auto px-0 md:px-6 grid md:grid-cols-[240px_1fr] gap-10 items-start">
...
function FilterPanel() {
  ...
  const qParam = searchParams.get("q")
    ...
    : { sizes: [], brands: [], sortBy: "po.created_at asc" };
          ...
          <AccordionContent>
            <div className="flex flex-wrap gap-1">
              {productCategories.map((category) => (
                <Link
                  key={category.name}
                  href={pathname === category.href ? "/browse" : category.href}
                  className={cn(
  ...
  return (
    <Sheet>
      <SheetTrigger asChild>
        <Button
          className="absolute top-[104px] right-0 text-white border-l border-gray-50 rounded-none md:hidden"
          variant="ghost"
        >
          <FilterIcon className="h-3 w-3" />
        </Button>
      </SheetTrigger>
      <SheetContent className="p-0 overflow-auto" hideXButton={true}>
        <FilterPanel />
      </SheetContent>
    </Sheet>
