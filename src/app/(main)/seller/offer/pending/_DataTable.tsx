...
export default function Dataable<TData, TValue>({
  ...
  const [publishProductOffer,{
    data: publishProductOfferData,
  }] = useMutation(SELLER_DASHBOARD__PUBLISH_PENDING_PRODUCT_OFFER);

  const handlePublish = useCallback(async (offer_ids : number[])=>{
    await publishProductOffer({
      variables: {
         offer_ids,
      },
    });
    ...
    <div className="rounded-md overflow-x-auto space-y-2">
      <div>
        {selectedOfferIds.length > 0 && (
          <Button onClick={()=>handlePublish(selectedOfferIds)}>
