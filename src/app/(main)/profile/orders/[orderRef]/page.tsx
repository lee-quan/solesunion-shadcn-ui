...
import { GET_ORDER_DETAILS } from "@/lib/graphql/queries/profileQueries";
      ...
      <div className="flex items-center gap-4">
        ...
        <h1 className="font-semibold text-lg md:text-xl">
          ...
          <span className="font-normal text-gray-500 dark:text-gray-400 ml-2 ">
            {!!data && ` on ${prettyDate(data?.userOrderDetail.created_at)}`}
          </span>
        </h1>
      </div>
      {loading ? (
                  ...
                  <TableRow className="border-none">
                    <TableCell className="font-medium" colSpan={4}>
                      ...
                      <Link
                        ...
                        className="underline"
                      >
                        {orderDetail.product.product_title}
                      </Link>
                    </TableCell>
                  </TableRow>
                  <TableRow>
                    ...
                    <TableCell>RM {price2d(orderDetail.price)}</TableCell>
                  </TableRow>
                </React.Fragment>
              )
            )}
    ...
    <Card>
      ...
      <CardContent className="grid gap-4">
        <div className="flex items-center">
          <div>Subtotal</div>
          <div className="ml-auto">RM {price2d(order.order_total / 1.03)}</div>
        </div>
        <div className="flex items-center">
          <div>Processing</div>
        ...
        <div className="flex items-center font-medium">
          <div>Total</div>
          <div className="ml-auto">RM {price2d(order.order_total)}</div>
        </div>
      </CardContent>
    </Card>
  );
